...
import { LocationsGetService } from '../components/locations/service/LocationsApiService';
...
const PageLocationsWithEmployeesReport = () => {
  ...
  const validateLocations = () => {
    if (!locationIds || locationIds.length === 0) {
      toast.error('Пожалуйста, выберите хотя бы одно помещение.');
      return false;
    }
    return true;
  };
  ...
  const handleShowReport = async () => {
    ...
    try {
      ...
      const htmlContent = await reportApiService.getHtml('locations-with-employees', reportDto);
  ...
  const handleDownloadReport = async (format) => {
    ...
    try {
      ...
      switch (format) {
        case 'pdf':
          await downloadFile(reportApiService.downloadPdf('locations-with-employees', reportDto), 'report.pdf');
          break;
        case 'docx':
          await downloadFile(reportApiService.downloadDocx('locations-with-employees', reportDto), 'report.docx');
          break;
        case 'xlsx':
          await downloadFile(reportApiService.downloadXlsx('locations-with-employees', reportDto), 'report.xlsx');
          break;
        case 'html': {
          try {
            const htmlContent = await reportApiService.getHtml('locations-with-employees', reportDto);
    ...
    <div className="row g-2">
      <div className="col-12 px-0">
        <div className="block d-flex justify-content-center fs-2 fw-bold admin-title mb-1">
          Помещения с сотрудниками
        </div>
      </div>

      <div className="col-12 mb-2 p-4 bg-light rounded">
        <div className="form-label fs-5 fw-medium mb-2">Выберите помещения:</div>
        <SearchableSelect
          ...
          isPagination
          className="w-100"
          onChange={(values) => setLocationIds(values)}
          required
        />
      </div>
...
export default PageLocationsWithEmployeesReport;